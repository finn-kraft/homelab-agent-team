...
MIGRATION_VERSION = "orchestrator-0001"
    ...
    def record_checkpoint(self, work: dict[str, Any], worker_id: str, result: Any) -> str:
        ...
        with self.connect() as connection:
            ...
            if success:
                ...
                return "complete"
            next_step_status = "changes_requested" if retryable else "needs_human"
            next_job_status = "running" if retryable else "needs_human"
            connection.execute(
            ...
            if not controlled:
                connection.execute(
                    "UPDATE jobs SET status=%s,current_phase=%s,updated_at=now() WHERE id=%s",
                    (next_job_status, "coding" if retryable else "needs_human", work["job_id"]),
                )
    ...
    def enforce_iteration_limit(self) -> int | None:
        ...
        with self.connect() as connection:
            ...
            if not row:
                return None
            connection.execute(
                """UPDATE jobs SET status='needs_human',current_phase='needs_human',
                human_notes=COALESCE(human_notes || E'\n','') ||
                  'Configured maximum planning iterations reached. Review the job before resuming.',
                updated_at=now() WHERE id=%s""", (row["id"],)
            )
            self._event(connection, row["id"], None, "human_input_requested", {
                "question": "The configured iteration ceiling was reached. Should this job be extended, re-scoped, or completed?",
            })
            return int(row["id"])
