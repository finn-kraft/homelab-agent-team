...
class CoderAgent:
    ...
    def run_task(self, task: Task) -> AgentResult:
        if task.attempt > self.max_attempts:
            result = AgentResult(Status.NEEDS_HUMAN, "retry limit reached",
                                 blocker="maximum attempts exceeded")
        ...
        try:
            ...
            status = Status.NEEDS_HUMAN if task.attempt >= self.max_attempts else Status.FAILED
            self.store.update_step(task.step_id, self.worker_id, status, blocker=str(exc))
            return AgentResult(status, "task did not reach review", blocker=str(exc))
